...
if (isset($_POST['alternate-booking'])) {
...
*/
    ...
    $html = '<div style="font-size:1rem"><p>Hello ' . $user['name'] . '!</p><p>Your alternative Appointment with Dr. ' . $doctor['name'] . ' has been confirmed!</p><p>Your appointment is scheduled on ' . $date_dd_mm_yyyy . ' at ' . $slot['time'] . '</p><p>Once again, we apologize for the inconvinience caused.</p><p>Use the button below to join the meeting at the scheduled time.</p><p style="padding-top:20px"><a href="' . $result->join_url . '" style="background-color:#0D6EFD;color:#fff;padding:10px 10px; text-decoration:none;border-radius:10px;">Join Meeting</a></p></div>';
